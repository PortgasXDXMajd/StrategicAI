...
export type HypothesisRootNode = Node<
  {
    node: NodeModel;
    layer: number;
  },
...
const HypothesisRootNode = ({ id, data }: NodeProps<HypothesisRootNode>) => {
    ...
    <NodeOptions key={id} node={data.node} layer={data.layer}>
      <div
        className={`flex items-center justify-between h-fit 
          min-w-[200px] max-w-[500px] border-1 rounded-md p-2
          bg-[#F9F9F9]
           dark:bg-[#181818]
...
export default HypothesisRootNode;
